    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        ...
        Api.Info.saveInfo({body}).then(res => {
          if (res.Ret === 0) {
            this.setState({ID: res.Data.ID,detail:res.Data,});
...
showModal = () => {
  ...
  render() {
    const {editStatus,detail ,channelList, uploadPicLoading, coverOne, coverTwo, coverThree, coverFour, uploadLoadingTwo, uploadLoadingFour, uploadLoadingThree, tags, inputVisible, inputValue, tagBox, id,preview} = this.state;
      ...
      <div className="articleEdit">
        ...
        <div className="article-left">
          <Form className="login-form">
            ...
            {this.state.value === "1" ? <FormItem {...formItemLayout} extra="封面大小不超过2M，画面清晰，建议尺寸比例为3：2"
                                                  style={{marginLeft: '25%'}}>
            ...
            {this.state.value === "0" ? <FormItem {...formItemLayout} extra="封面大小不超过2M，画面清晰，建议尺寸比例为3：2"
                                                  style={{marginLeft: '25%'}}>
            ...
            <FormItem {...tailFormItemLayout}>
            	{detail?<Button onClick={() => this.showModal()} className="preview-btn">预览</Button>:""}
	        ...
	        >
	          {preview?<GetQrcode contentID={detail.ID} contentType={detail.ContentType}/>:""}
	        </Modal>
          </Form>
