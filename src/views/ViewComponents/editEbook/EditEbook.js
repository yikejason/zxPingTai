...
import GetQrcode from "../../publicComponents/getQrcode"; 
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (state.filekey.length !== 0 || state.fileList.length !== 0) {
          ...
          let trailFile;
//        console.log(values.trailFile)
          if(values.trailFile){
          	if(this.state.read==='0' && values.trailFile.length>0) {
	            trailFile = {
...
//        console.log(body)
          if(body.File.Url&&body.File.Url!=="undefined"){
          	if(body.TrailFile){
          		if(body.TrailFile.Url!=="undefined"){
          			Api.Content.saveEbook({body}).then(res => {
			            if (res.Ret === 0) {
			              this.setState({ID: res.Data.ID,contentType:res.Data.ContentType});
...
//  console.log(isVideo && isLtSize)
  ...
  render(){
    ...
    const {tags,editStatus,fileList,filekey,id,ID,contentType,preview} =this.state;
      ...
      <div className="editEbook">
        ...
        <div className="Ebook-left">
          <Form  className="login-form">
            ...
            <FormItem label="封面" {...formItemLayout}  extra="封面大小不超过2M，画面清晰，建议尺寸比例为3：4">
            ...
            <FormItem{...formItemLayout} label="电子书文件"  extra="可支持电子书文件格式：PDF,TXT">
            ...
            {this.state.read === "0" && <FormItem label='试读文件' {...formItemLayout} extra="可支持电子书文件格式：PDF,TXT">
            ...
            <FormItem {...tailFormItemLayout}>
              {contentType?<Button onClick={() => this.showModal()} className="preview-btn">预览</Button>:""}
	        ...
	        >
	          {preview?<GetQrcode contentID={ID} contentType={contentType}/>:""}
	        </Modal>
          </Form>
        </div>
        {/*预览编辑页面*/}
