    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        ...
        Api.Info.savePropaganda({body}).then(res => {
          if (res.Ret === 0) {
            this.setState({ID: res.Data.ID,contentType:res.Data.ContentType,RefResigsterFields:res.Data.RefResigsterFields});
...
showModal = () => {
  ...
  render() {
    ...
    const {channelList, tags, editStatus,id,ID,contentType,preview} = this.state;
