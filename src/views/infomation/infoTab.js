...
class InfoTab extends Component {
                            ...
                            <div>
                                ...
                                <span className="button-right">
                                    {(record.Status === 8 && record.ContentType === 14) || (record.Status === 11 && record.ContentType === 14) ?
                                        <Button onClick={() => this.exportUser(record.ID)} className="operation-btn">导出用户登记表</Button> : null}
    ...
    exportUser=(contentID)=>{
    		const token = Token.getUserToken();
	  		window.open(`http://api.test.ucuxin.com/ZX/v3/PublicityGW/Export?token=${token}&contentID=${contentID}`)
