...
class Notice extends Component {
  ...
  //确定
  sure = () => {
    const {Notice,listData,isLimite} = this.state;
    var flag=0;
    if(isLimite){
    	flag=1;
    }
  ...
  getData = (value,data)=>{
    this.setState({visible:value,listData:data,delThumb:false},
      ()=>{
  ...
  render() {
    ...
    const {isLimite,Notice,delThumb} = this.state;
    ...
    return (
      <div className="message-notice" style={{width:"70%"}}>
        ...
        <Modal
          ...
          footer={null}
        >
          <ZxTable delThumb={delThumb} getData={this.getData}/>
        </Modal>
        ...
        <FormItem label="关联内容" {...formItemLayout}>
          ...
          <Button  onClick={()=>this.delete()}>	清除关联内容</Button>
          <br/>{<div style={{display:delThumb?"none":"block"}}><span>{this.handleType(Notice.ContentType)}&nbsp;:<span>{Notice.Title}</span></span></div>}
        </FormItem>
