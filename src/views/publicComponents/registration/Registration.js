...
class Registration extends Component {
    ...
    componentWillReceiveProps(nextProps){
        if(nextProps.RefResigsterFields && nextProps.RefResigsterFields !== this.props.RefResigsterFields){
            this.setState(
                {data: nextProps.RefResigsterFields});
        }
    }
    //添加登记表
    handleAdd = () =>{
    ...
    handleEditModal = () =>{
     ...
     compare=(value1,value2)=>{
		if (value1.SNO < value2.SNO){
		return -1;
		}else if (value1.SNO > value2.SNO){
			return 1;
		}else{
			return 0;
		}
    ...
    handleEditOk = () =>{
        const {deleteData,valueList} = this.state;
    ...
    render () {
        let {visible,type,showVisible} = this.state;
        var data=this.state.data;
                    ...
                    <Form>
                       <FormItem label="配置字段" {...formItemLayout}>
                        ...
                        <FormItem label="排序" {...formItemLayout}>
                            ...
                            <Input placeholder="请输入排列顺序" type="number"/>
                            )}
                        </FormItem>
                    </Form>
                </Modal>
                    ...
                    <Form>
                        ...
                        <FormItem label="" {...formItemLayout}>
                            {getFieldDecorator('ID', {
                                rules: [{ required: false, message: '请输入排列顺序'}],
                            })(
                            <Input style={{display:"none"}} placeholder="请输入排列顺序"/>
                            )}
                        </FormItem>
                    </Form>
                </Modal>
